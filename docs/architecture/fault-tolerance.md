...
| Distributed rate limiting | `express-rate-limit` + Redis store | `middleware/rateLimiter.ts` |
| Idempotent processing | Status check before fulfillment | `workers/processors/fulfillOrderProcessor.ts` |
| Row-level locking | `SELECT ... FOR UPDATE` | `fulfillOrderWorker.ts`, `admin.ts` |
| Skip locked rows | `FOR UPDATE SKIP LOCKED` | `workers/processors/expiryProcessor.ts` |
| Compensation logic | Revert to `reserved` on payment failure | `routes/products.ts` |
| Distributed locking | Redis `SET NX EX` | `workers/processors/cleanupProcessor.ts` |
| Automatic retry | BullMQ 3 attempts, 1s backoff | `routes/webhook.ts` job config |
| Webhook retries | Return `500` on processing failure | `routes/webhook.ts` |
| Identity validation | Stripe metadata vs DB user ID | `routes/webhook.ts` |
| Graceful degradation | Sync failure does not block startup | `src/server.ts` |
...
**Where it is implemented**: `middleware/rateLimiter.ts` — two limiters: `reserveLimiter` (10 requests per 15 minutes) and `paymentLimiter` (3 requests per 1 minute). Rate limit state is stored in Redis, keyed by the authenticated user's ID (extracted from the JWT), not by IP address.
...
**Where it is implemented**: `fulfillOrderProcessor.ts` — before any database write, the processor queries the order's current `status`. If it is already `'completed'`, the processor logs a warning and exits without making any changes.
...
**Where it is implemented**: `fulfillOrderProcessor.ts` and `admin.ts` (cancel endpoint). Both open a `BEGIN` transaction and immediately issue `SELECT ... FOR UPDATE` on the order row before reading its status or writing any changes.
...
**Where it is implemented**: `expiryProcessor.ts` — `SELECT * FROM orders WHERE expires_at < NOW() AND status = 'reserved' FOR UPDATE SKIP LOCKED`.
...
**Where it is implemented**: `routes/products.ts` — in the `POST /product/create-payment-intent` error handler. If the Stripe PaymentIntent creation fails, the handler reverts the order status from `payment_pending` back to `reserved` so the expiry worker can eventually clean it up.
...
**Where it is implemented**: `cleanupProcessor.ts` — `SET cleanup-lock running NX EX 300`. `NX` means the key is only set if it does not already exist (atomic compare-and-set). `EX 300` sets a 300-second TTL so the lock automatically expires if the process crashes mid-cleanup.
...
**Where it is implemented**: `routes/webhook.ts` — `purchaseQueue.add('fulfill-order', data, { attempts: 3, backoff: { type: 'fixed', delay: 1000 }, removeOnComplete: true, removeOnFail: false })`.
...
**Where it is implemented**: `routes/webhook.ts` — uncaught errors in the handler return HTTP `500`.
...
**Where it is implemented**: `routes/webhook.ts` — after verifying the webhook signature, the handler queries the database for the order using the `payment_intent_id` from the event and compares the database `user_id` with `event.data.object.metadata.user_id`.
...
**Where it is implemented**: `src/server.ts` — the `syncInventoryToRedis()` call is wrapped so that a failure logs a warning but does not throw, allowing `app.listen()` to proceed.
